...
// PORTFOLIO BUILD: this copy is embedded in the portfolio with no backend
// behind it. Leaving the call in place is actively harmful here — a dev server
// that answers unknown paths with index.html returns status 200, which the
// code below reads as a successful API response and sends the game down its
// online path with empty config. Fail fast so every caller takes the guest
// fallback it already implements.
const EMBEDDED_OFFLINE = true;

async function api(path, { method = 'GET', body } = {}) {
  if (EMBEDDED_OFFLINE) {
    throw Object.assign(new Error('No backend in the embedded build'), { status: 0 });
  }
